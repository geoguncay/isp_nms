...
def daily_suspension_check():
    """
    Busca clientes activos con al menos una factura pendiente/vencida cuya fecha_vencimiento,
    más los días de gracia configurados (suspension_retraso_dias), ya se cumplió. Los suspende
    de forma automática en la base de datos, en MikroTik y les envía una notificación.
    Respeta el interruptor "Suspensión automática" (suspension_automatica) de Ajustes.
    """
    ...
    try:
        ...
        if not cfg.suspension_automatica:
            logger.info("Suspensión automática desactivada en Ajustes. Tarea omitida.")
            return

        now = datetime.now(timezone.utc)
        gracia = timedelta(days=cfg.suspension_retraso_dias or 0)
        ...
        for client in active_clients:
            ...
            overdue_invoice = (
                db.query(Invoice)
                .filter(Invoice.cliente_id == client.id, Invoice.estado.in_(("pendiente", "vencido")))
            ...
            due = overdue_invoice.fecha_vencimiento
            ...
            reason = (
                f"Mora de pago: factura del periodo {overdue_invoice.periodo} "
                f"vencida el {due.strftime('%Y-%m-%d')} (gracia de {cfg.suspension_retraso_dias} días cumplida)"
            )
            ...
            try:
                ...
                if cfg.suspension_notify_suspendido:
                    try:
